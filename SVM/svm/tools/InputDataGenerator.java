...
public class InputDataGenerator extends Dialog{
	...
	void generateData(){
		...
		if(liniar.getState()){
			generateLiniarSeparatedData(marg);
			//generateMajoritarData();
		}else{
			generateNonLiniarSeparatedData(marg, 50);
			//generateRandomData();
		}
	}
	...
	void generateLiniarSeparatedData(float marg){ //the margin: marg>=0
		int N = Integer.parseInt(vectors_count.getText());
		int n = Integer.parseInt(attributes_count.getText());
		int MIN = Integer.parseInt(min.getText());
		int MAX = Integer.parseInt(max.getText());
	...
	void generateNonLiniarSeparatedData(float marg, int procent){ //the margin: marg>=0; procent = the procent of non linear separated data
		int N = Integer.parseInt(vectors_count.getText());
		int n = Integer.parseInt(attributes_count.getText());
		int MIN = Integer.parseInt(min.getText());
		int MAX = Integer.parseInt(max.getText());
		...
		if(N > 1 && n > 1 && MIN < MAX){
			...
			w[n] = max*(MIN + (float)Math.random()*(MAX-MIN));
			if(procent < 0 || procent > 100) procent = 0;
			int N1 = N - (int)((procent*N)/100);
			for(int k=0; k<N1; k++){
				while(true){
					String s = "";
					float[] x = new float[n];
			...
			for(int k=N1; k<N; k++){
				while(true){
					...
					if(z <= -max*marg || z >= max*marg)continue;
					for(int i=0; i<n; i++) s += x[i] + ",";				
					s += y + "\n";	
					ta.append(s);
					break;

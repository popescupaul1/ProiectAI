...
public class Options extends Dialog implements AdjustmentListener{
	...
	public Options(SVM svm){
		...
		y+=30;
		demax_label = new Label("\u0394emax :");
		demax_label.setBounds(25,y,100,20);
		demax_label.setForeground(Color.white);
		add(demax_label);
		demax_tf = new TextField("");
		demax_tf.setForeground(Color.black);
		demax_tf.setBounds(200,y,90,20);
		add(demax_tf);
		demax_tf.setText(""+svm.settings.demax);	
		demax_tf.enable(false);
	...
	public boolean handleEvent(Event e){
		if(e.id==Event.WINDOW_DESTROY){
			...
			double demax = svm.settings.demax_default;
			try{
				demax = Double.parseDouble(demax_tf.getText());
			}
			catch(NumberFormatException nfe){}
			svm.settings.demax = demax;			
